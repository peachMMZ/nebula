...
jwt:
  ...
  accessTokenDuration: 7200      # 2 小时
  refreshTokenDuration: 604800   # 7 天
...
# 产物: dist/nebula-server 或 dist/nebula-server.exe
```
...
export SERVER_ADDRESS=:9050
export DATABASE_DSN=/app/data/nebula.db
./nebula-server
```
...
更新检查
POST   /api/update/check            # 检查更新（客户端调用）
```
...
```bash
# 1. 创建目录结构
mkdir -p config uploads data logs

# 2. 创建配置文件
cat > config/nebula.yaml << 'EOF'
server:
  address: ":9050"
database:
  dsn: "/app/data/nebula.db"
storage:
  type: local
...
docker run -d \
  ...
  -v $(pwd)/data:/app/data \
  -v $(pwd)/logs:/app/logs \
  ghcr.io/peachmmz/nebula:latest
```
...
| `CONFIG_FILE` | 配置文件路径 | config/nebula.yaml |
| `SERVER_ADDRESS` | 服务监听地址 | :9050 |
| `DATABASE_DSN` | 数据库连接字符串 | data/nebula.db |
| `STORAGE_TYPE` | 存储类型 (local) | local |
| `STORAGE_BASE_PATH` | 存储基础路径 | ./uploads |
| `STORAGE_BASE_URL` | 文件访问基础 URL | http://localhost:9050/files |
